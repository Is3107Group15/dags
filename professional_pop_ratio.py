...
DATA_SOURCE_FILE = 'data/healthcare-professional-to-population-ratio/healthcare-professional-to-population-ratio.csv'

@task
def extract_data(source_file: str) -> str:
    gcs_hook = GCSHook()
    bucket_name = 'asia-southeast1-airflow3107-aeb72f53-bucket'
    local_source_file = '/tmp/professional_pop_ratio.csv'
...
with DAG(
    'professional_pop_ratio',
    default_args=default_args,
    start_date=days_ago(1),
    schedule_interval='0 0 1 1 *',  # Run once a year on January 1st at 00:00
    tags=['project']
) as dag:
    dag.doc_md = "extract number of professionals to population data from gcs and load to data big query (datawarehouse)"

    extract_data_task = extract_data(source_file=DATA_SOURCE_FILE)

    upload_data_task = upload_file_to_gcs(local_file=extract_data_task, remote_file_path='data/healthcare-professional-to-population-ratio')
    ...
    load_data = GCSToBigQueryOperator(
        ...
        field_delimiter=',',
        destination_project_dataset_table='project.professional_pop_ratio',
        skip_leading_rows=1,
        schema_fields=[
            {'name': 'year', 'type': 'INTEGER'},
            {'name': 'profession', 'type': 'STRING'},
            {'name': 'no_per_1000', 'type': 'STRING'},
            {'name': 'ratio_to_population', 'type': 'STRING'}
        ],
