...
DATA_SOURCE_FILE = 'data/number-of-nurses-and-midwives/number-of-nurses-and-midwives.csv'

@task
def extract_data(source_file: str) -> str:
    gcs_hook = GCSHook()
    bucket_name = 'asia-southeast1-airflow3107-aeb72f53-bucket'
    local_source_file = '/tmp/num_nurses.csv'
...
with DAG(
    'num_nurses_2010_onwards',
    default_args=default_args,
    start_date=days_ago(1),
    schedule_interval='0 0 1 1 *',  # Run once a year on January 1st at 00:00
    tags=['project']
) as dag:
    dag.doc_md = "extract number of nurses and midwifes data from gcs and load to data big query (datawarehouse)"
    ...
    remote_filtered_data = f'data/number-of-nurses-and-midwives/filtered/{filter_data_task}'
    upload_data_task = upload_file_to_gcs(local_file=filter_data_task, remote_file_path='data/number-of-nurses-and-midwives/filtered')
    ...
    load_data = GCSToBigQueryOperator(
        ...
        field_delimiter=',',
        destination_project_dataset_table='project.num_nurses',
        skip_leading_rows=1,
        schema_fields=[
