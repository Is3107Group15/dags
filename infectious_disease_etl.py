...
DATA_SOURCE_FILE = 'data/weekly-infectious-disease-bulletin-cases/weekly-infectious-disease-bulletin-cases.csv'

@task
def extract_data(source_file: str) -> str:
    gcs_hook = GCSHook()
    bucket_name = 'asia-southeast1-airflow3107-aeb72f53-bucket'
    local_source_file = '/tmp/weekly-infectious-disease.csv' 
...
with DAG(
    ...
    dag.doc_md = "extract weekly infectious disease data from gcs and load to data big query (datawarehouse)"
    ...
    load_data = GCSToBigQueryOperator(
        ...
        field_delimiter=',',
        destination_project_dataset_table='project.weekly-infectious-disease',
        skip_leading_rows=1,
        schema_fields=[
            {'name': 'epi_week', 'type': 'STRING'},
            {'name': 'disease', 'type': 'STRING'},
            {'name': 'no_of_cases', 'type': 'INTEGER'},
