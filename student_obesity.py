...
DATA_SOURCE_FILE = 'data/common-health-problems-of-students-examined-obesity-annual/common-health-problems-of-students-examined-overweight-annual.csv'

@task
def extract_data(source_file: str) -> str:
    gcs_hook = GCSHook()
    bucket_name = 'asia-southeast1-airflow3107-aeb72f53-bucket'
    local_source_file = '/tmp/student_obesity.csv'
...
with DAG(
    ...
    dag.doc_md = "extract student obesity data from gcs and load to data big query (datawarehouse)"

    extract_data_task = extract_data(source_file=DATA_SOURCE_FILE)

    upload_data_task = upload_file_to_gcs(local_file=extract_data_task, remote_file_path='data/common-health-problems-of-students-examined-obesity-annual')
    ...
    load_data = GCSToBigQueryOperator(
        ...
        field_delimiter=',',
        destination_project_dataset_table='project.student_obesity',
        skip_leading_rows=1,
        schema_fields=[
            {'name': 'year', 'type': 'INTEGER'},
            {'name': 'age_group', 'type': 'STRING'},
            {'name': 'gender', 'type': 'STRING'},
            {'name': 'per_10000_examined', 'type': 'INTEGER'}
        ],
