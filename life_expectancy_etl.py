...
DATA_SOURCE_FILE = 'data/health-adjusted-life-expectancy/health-adjusted-life-expectancy-at-birth.csv'

@task
def extract_data(source_file: str) -> str:
    gcs_hook = GCSHook()
    bucket_name = 'asia-southeast1-airflow3107-aeb72f53-bucket'
    local_source_file = '/tmp/life_expectancy.csv'
...
with DAG(
    ...
    dag.doc_md = "extract life expectancy data from gcs and load to data big query (datawarehouse)"

    extract_data_task = extract_data(source_file=DATA_SOURCE_FILE)

    upload_data_task = upload_file_to_gcs(local_file=extract_data_task, remote_file_path='data/health-adjusted-life-expectancy')
    ...
    load_data = GCSToBigQueryOperator(
        ...
        field_delimiter=',',
        destination_project_dataset_table='project.life_expectancy',
        skip_leading_rows=1,
        schema_fields=[
            {'name': 'year', 'type': 'INTEGER'},
            {'name': 'sex', 'type': 'STRING'},
            {'name': 'hale_years', 'type': 'FLOAT'}
        ],
