...
DATA_SOURCE_FILE = 'data/health-facilities/health-facilities-and-beds-in-inpatient-facilities-public-not-for-profit-private.csv'

@task
def extract_data(source_file: str) -> str:
    gcs_hook = GCSHook()
    bucket_name = 'asia-southeast1-airflow3107-aeb72f53-bucket'
    local_source_file = '/tmp/health_facilities.csv'
...
with DAG(
    'health_facil_2010_onwards',
    default_args=default_args,
    start_date=days_ago(1),
    schedule_interval='0 0 1 1 *',  # Run once a year on January 1st at 00:00
    tags=['project']
) as dag:
    dag.doc_md = "extract healthcare facility data from gcs and load to data big query (datawarehouse)"
    ...
    remote_filtered_data = f'data/health-facilities/filtered/{filter_data_task}'
    upload_data_task = upload_file_to_gcs(local_file=filter_data_task, remote_file_path='data/health-facilities/filtered')
    ...
    load_data = GCSToBigQueryOperator(
        ...
        field_delimiter=',',
        destination_project_dataset_table='project.health_facilities',
        skip_leading_rows=1,
        schema_fields=[
            {'name': 'year', 'type': 'INTEGER'},
            {'name': 'institution_type', 'type': 'STRING'},
            {'name': 'facility_type_a', 'type': 'STRING'},
            {'name': 'public_private', 'type': 'STRING'},
            {'name': 'no_of_facilities', 'type': 'INTEGER'},
            {'name': 'no_beds', 'type': 'INTEGER'}
        ],
