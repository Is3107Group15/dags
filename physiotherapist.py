...
DATA_SOURCE_FILE = 'data/number-of-physiotherapists/number-of-physiotherapists.csv'

@task
def extract_data(source_file: str) -> str:
    gcs_hook = GCSHook()
    bucket_name = 'asia-southeast1-airflow3107-aeb72f53-bucket'
    local_source_file = '/tmp/num_physiotherapists.csv'
...
with DAG(
    'num_physiotherapists',
    default_args=default_args,
    start_date=days_ago(1),
    schedule_interval='0 0 1 1 *',  # Run once a year on January 1st at 00:00
    tags=['project']
) as dag:
    dag.doc_md = "extract number of physiotherapist data from gcs and load to data big query (datawarehouse)"
    ...
    remote_filtered_data = f'data/number-of-physiotherapists/filtered/{filter_data_task}'
    upload_data_task = upload_file_to_gcs(local_file=filter_data_task, remote_file_path='data/number-of-physiotherapists/filtered')
    ...
    load_data = GCSToBigQueryOperator(
        ...
        field_delimiter=',',
        destination_project_dataset_table='project.num_physiotherapists',
        skip_leading_rows=1,
        schema_fields=[
